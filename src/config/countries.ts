...
/** Kirill o'zbekcha: lotin nomlarini transliteratsiya qilamiz (brauzer ICU'sida "uz-Cyrl" deyarli yo'q). */
const UZ_CYRL_OVERRIDES: Record<string, string> = {
  FR: "Франция",
  MS: "Монтсеррат",
  YT: "Майотта",
  VN: "Вьетнам",
...
const uzCyrlCache = new Map<string, string>();

function uzCyrlName(code: string): string | null {
  const latin = UZ_COUNTRY_NAMES[code];
  if (!latin) return null;
  let v = uzCyrlCache.get(code);
  if (!v) {
    v = UZ_CYRL_OVERRIDES[code] ?? uzLatinToCyrillic(latin);
    uzCyrlCache.set(code, v);
  }
  return v;
}
...
export function searchCountries(query: string, lang = "uz", limit = 40): string[] {
  ...
  for (const code of COUNTRY_CODES) {
    const extra = lang === "uz-cyrl" ? [countryName(code, "uz")] : [];
    const names = [countryName(code, lang), ...extra, countryName(code, "en"), code].map((n) => n.toLowerCase());

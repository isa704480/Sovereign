...
export function install() {
  ...
  const state = () => ({
    ...
    offline: false, platform: "win32", settings: { ...settings }, recent: ["D:\\Projects\\demo-shop", "D:\\Projects\\api-server"], history: [...history], task: null, busy: false, update: { state: "idle" },
  });
  ...
  window.sovereign = {
    ...
    updates: { check: async () => ({ state: "no-release" }), download: async () => ({ state: "idle" }), install: async () => ({ ok: true }) },
